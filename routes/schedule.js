    ...
    (err, users) => {
      ...
      db.all("SELECT * FROM juniorHighClasses", (err, juniors) => {
        ...
        db.all("SELECT * FROM seniorHighClasses", (err, seniors) => {
          ...
          const loginUserClass = users.class;
          const instructorClasses = users.junior_classes + users.senior_classes;
          console.log(`All instructor classes data:\n`, instructorClasses);
                  ...
                  db.get(
                    ...
                    (err, authRows) => {
                      ...
                      const query = `
                        ...
                        WHERE schedules.class_id = ? AND schedules.school_id = ?
                        ORDER BY schedules.class_id, schedules.day, schedules.start_time;
                        `;

                      db.all(query, [loginUserClass, loginAdminSchoolID],(err, schedules) => {
                        if (err) {
                          return res
                            .status(500)
                        ...
                        const instructorClasses = [
                          ...new Set([
                            ...(users.junior_classes ? users.junior_classes.split(',') : []),
                            ...(users.senior_classes ? users.senior_classes.split(',') : [])
                          ])
                        ];
                        
                        console.log(`Instructor's classes:`, instructorClasses);
                        ...
                        JOIN subjects ON schedules.instructor_id = subjects.instructor_id
                        WHERE 
                          (juniorHighClasses.class_name IN (${instructorClasses.map(() => '?').join(',')})
                           OR seniorHighClasses.class_name IN (${instructorClasses.map(() => '?').join(',')}))
                          AND schedules.school_id = ?
                        ORDER BY schedules.class_id, schedules.day, schedules.start_time;
                      `;
                      
                      // Dynamically pass the parameters
                      db.all(
                        instructorQuery,
                        [...instructorClasses, ...instructorClasses, loginAdminSchoolID],
                        (err, instructorSchedules) => {
                          if (err) {
                            console.error("Error fetching schedules:", err);
                            return res.status(500).send("Error fetching schedules.");
                          }
                          console.log("Instructor Schedules", instructorSchedules);
                      
                          // Group schedules by class and day
                          const groupedInstructorSchedules = {};
                          instructorSchedules.forEach((instructorSchedule) => {
                            const key = `${instructorSchedule.class_name}-${instructorSchedule.day}`;
                            if (!groupedInstructorSchedules[key]) {
                              groupedInstructorSchedules[key] = [];
                            }
                            groupedInstructorSchedules[key].push(instructorSchedule);
                          });
